...
const USERS_LIST_TTL = 300;
// Get all users
export const getAllReaders = async (req: Request, res: Response, next: NextFunction): Promise<void> => {
  try {
...
export const getReaderById = async (req: Request, res: Response, next: NextFunction): Promise<void> => {
  try {
    const { id } = req.query;
    ...
    next(new AppError("Failed to fech user", 500));
  }
}


// Delete user with cache invalidation
export const deleteReader = async (req: Request, res: Response, next: NextFunction): Promise<void> => {
  try {
    const { id } = req.params;
    const reader = await Reader.findByIdAndDelete(id).lean();
...
export const getReaderStats = async (req: Request, res: Response, next: NextFunction): Promise<void> => {
  try {
    const { id } = req.params;
    ...
    // const inter
    const stats = {
      contentCount: await Content.countDocuments({ businessId: id }), 
...
export const calculateEngagementRate = async (readerId: string): Promise<number> => {
  const [interactionCount, contentCount] = await Promise.all([
    Interaction.countDocuments({ userId: readerId }),
